...
export default function MatchCard({ match }: { match: MatchView }) {
  ...
  return (
    <article className="app-card match-card group overflow-hidden rounded-[30px] border border-white/10 bg-[#141120] shadow-[0_24px_64px_rgba(0,0,0,0.42)] transition duration-300 hover:-translate-y-0.5 hover:border-[#9A63F1]">
      <div className="match-photo-area relative h-[520px]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6">
          <h3 className="mt-2 text-[56px] font-semibold leading-[0.92] tracking-[-0.025em] text-[#F7F8FF]" style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>
            {match.matchedProfile.firstName}
            {typeof match.matchedProfile.age === 'number' ? `, ${match.matchedProfile.age}` : ''}
          </h3>
          <p className="mt-1 text-lg text-white/80">✨ {match.matchedProfile.location || 'Location not shared yet'}</p>
          <p className="mt-0.5 text-sm text-white/85">{truncate(match.matchedProfile.bio || 'Intentional dater on Vinculo.', 82)}</p>
        </div>
      </div>
